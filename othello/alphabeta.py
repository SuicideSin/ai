...
def alphabeta(board, depth, alpha, beta, onside, side):
    ...
    opposite = oppositeSide[side]
    if depth == 0 or (len(possible['X']) == 0 and len(possible['O']) == 0):
        return 0.001 * board.count(side) + len(possible[side]) + 10 * len([i for i in range(64) if board[i] == side and i in [0, 7, 56, 63]]) - 10 * len([i for i in range(64) if board[i] == opposite and i in [0, 7, 56, 63]])
    if onside:
        v = float("-inf")
        for pos in possible[side]:
            child = flipBoard(board, pos, side)
